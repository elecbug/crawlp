package aaai

import (
	"fmt"
	"net/http"
	"strings"

	"github.com/elecbug/crawlp/internal/provider"
)

type Downloader struct{}

func NewDownloader() *Downloader {
	return &Downloader{}
}

func (d *Downloader) ID() string {
	return "aaai"
}

func (d *Downloader) Name() string {
	return "AAAI Publications"
}

func (d *Downloader) MatchDOI(doi string) bool {
	normalized := strings.ToLower(
		strings.TrimSpace(doi),
	)

	return strings.HasPrefix(
		normalized,
		aaaiDOIPrefix,
	)
}

func (d *Downloader) Download(
	cli *http.Client,
	doi string,
	outputDir string,
) (provider.Result, error) {
	info, err := resolveAAAIDocument(cli, doi)
	if err != nil {
		return provider.Result{}, fmt.Errorf(
			"failed to resolve AAAI document: %w",
			err,
		)
	}

	outputPath, err := downloadAAAIPDF(
		cli,
		info,
		outputDir,
	)
	if err != nil {
		return provider.Result{}, fmt.Errorf(
			"failed to download AAAI document: %w",
			err,
		)
	}

	return provider.Result{
		ProviderID:   d.ID(),
		ProviderName: d.Name(),
		Title:        info.Title,
		Identifier:   info.Identifier,
		LandingURL:   info.LandingURL,
		OutputPath:   outputPath,
	}, nil
}
